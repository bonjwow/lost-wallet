...
# regression (for whatever reason, can't do fixed effects for city + institution using plm without issues)

s8_main_result <- lm(response ~ cond, data = s8)
summary(s8_main_result)
  
# regression by country

s8_by_country <- lm(response ~ cond + male + above40 + coworkers + computer + other_bystanders, data = s8)
summary(s8_by_country)
