...
Figure \@ref(fig:figReportedByCountry) is a replica of the dumbbell plot included in the original research paper, which shows share of returned (reported) wallets in NoMoney and Money conditions by country. And, Figure \@ref(fig:figMonetaryStakes) is a replica of the line plot included in the paper, which showsshare of returned (reported) wallets in the three conditions; NoMoeny, Money, and BigMoney. We filtered the raw dataset using the `dplyr` package [@citeDplyr] and display the data in a dumbbell plot and line plot using the `ggplot` [@citeGgplot2]. As can be seen in the figures, people were more likely to return wallets with more money in most of the countries selected for the research, and the reporting rate increases if wallets contain bigger money. If so, can we say that people tend to return wallets contained bigger money in "most countries"?
