...
stargazer(s8_main_result, s8_money_result, 
          ...
          column.labels = c("No Money", "Money"))

```

```{r S9, echo = FALSE, warning = FALSE, message = FALSE}
### conditions
# 0 - no money, 1 - money, 2 - big money, 3 - money no key.
...
s9_main_result <- lm(response ~ cond, data = s9_main)

# UK
s9_uk <- s9_main %>%
  filter(country == 39)
s9_uk_result <- lm(response ~ cond, data = s9_uk)

# Poland
s9_poland <- s9_main %>%
  filter(country == 27)
  
s9_poland_result <- lm(response ~ cond, data = s9_poland)

# USA
s9_usa <- s9_main %>%
  filter(country == 40)
s9_usa_result <- lm(response ~ cond, data = s9_usa)
...
stargazer(s9_main_result, s9_uk_result, s9_poland_result, s9_usa_result, 
          ...
          type = 'latex', 
          dep.var.labels = "Response", 
          covariate.labels = c("Money", "Constant"),
          column.labels = c("UK, Poland, and US", "United Kingdom", "Poland", "United States"))
```

```{r S10, echo = FALSE, warning = FALSE, message = FALSE}
### conditions
# 0 - no money, 1 - money, 2 - big money, 3 - money no key.
